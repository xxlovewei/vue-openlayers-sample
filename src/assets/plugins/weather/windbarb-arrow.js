function Arrow(options) {
    ...
    function loadArrowImg() {//
        ...
        arrowImg.src = "/static/dist/img/weather/arrow.png";
    }
    ...
    function loadBarbImg() {
        ...
        imgObj["2"].src = "/static/dist/img/weather/wind/2.png";
        for (let i = 3; i < 24; i++) {
            imgObj[i] = document.createElement("img");
            imgObj[i].src = "/static/dist/img/weather/wind/" + i + ".png";
            //镜像版
            imgObj[i+"-m"] = document.createElement("img");
            imgObj[i+"-m"].src = "/static/dist/img/weather/wind/" + i + "-m.png";
        }
    }
    ...
    function draw(data) {
        ...
        let viewportExtent = OlCommon.getViewportExtent_EPSG4326();
        let viewportMinLat = viewportExtent[1];
        let viewportMaxLat = viewportExtent[3];

        // 计算0-360经度的屏幕宽度
        let earthWidth = OlCommon.getWorldPixelWidth();

        // 获取浏览器视窗内最小经度、最大经度
        let extent = OlCommon.getViewportExtent_EPSG4326();
        let leftLon = extent[0];
        let rightLon = extent[2];

        // 计算视窗内有几个世界宽度
        let worldWidthNum = OlCommon.getWorldMaxNum(leftLon, rightLon);

        // 计算第一屏世界需要在0-360基础上的宽度偏移量
        let firstOffsetX = OlCommon.getFirstWordOffsetX(leftLon, earthWidth);

        let productType = data.productType;
        let isUV = productType == 'wind' || productType == 'oceanCurrent';
        let key = data.key;
        let data = data.srcData;
        ...
        let uarray = data[0].data;
        let varray = isUV ? data[1].data : null;
        ...
        for (let y = 0; y < ny; y = y + d_dy) {
            ...
            for (let x = 0; x < nx; x = x + d_dx) {
                ...
                switch (productType) {
                    case "wind":
                        let sd = getWindSpeedAndDirection(ut, vt);// 获取速度和方向
                        dtemp = (parseFloat(180)+parseFloat(sd.direction));
                        let imgFoot = OlCommon.getWindScaleBySpeed(sd.speed);//风速转风级
                ...
                // 坐标转换
                let pix = OlCommon.getPixelFromLngLat([lo, la]);
                let pix_x = pix.x;
                let pix_y = pix.y;
